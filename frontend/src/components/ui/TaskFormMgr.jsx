...
import { getTeamMembersByProjectId, createTask } from "../../services/api.js";
...
export default function TaskFormMgr({ projectId, projectName, onCancel, onCreated }) {
  ...
  const [formData, setFormData] = useState({
    ...
    status: "To Do",
    priority: "Low",
    deadline: dayjs().tz().format("YYYY-MM-DDTHH:mm"), // SG local
    createdBy: user.id,
    attachments: [],
  });

  useEffect(() => {
    if (projectId && projectId !== formData.assignedProject) {
      setFormData((prev) => ({ ...prev, assignedProject: projectId }));
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const data = await getTeamMembersByProjectId(projectId);
  ...
  async function handleSubmit(e) {
    ...
    setError(null);
    try {
      if (!formData.assignedProject) throw new Error("No project selected.");
      const deadlineIso = dayjs
        .tz(formData.deadline, "YYYY-MM-DDTHH:mm", "Asia/Singapore")
        .toISOString();

      const payload = { ...formData, deadline: deadlineIso };
  ...
  useEffect(() => {
    function handleClickOutside(ev) {
      if (!ev.target.closest(".assignee-dropdown-container")) {
  ...
  return (
    <div className="w-[min(95vw,900px)]">
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="lg:col-span-2 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 hover:border-gray-400 transition-colors">
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">Upload files, images, or documents</p>
                    </div>
                ...
                <div className="space-y-4 w-full max-w-[280px]">
                  <div>
                    ...
                    <div className="inline-flex items-center rounded-lg border px-3 py-2 text-sm bg-gray-50 text-gray-700">
                      {projectName || "Selected project"}
                    </div>
                    <input type="hidden" name="assignedProject" value={formData.assignedProject} />
                  </div>
                  ...
                  <div className="assignee-dropdown-container relative">
                        ...
                        <div className="grid grid-cols-2 gap-1 max-h-20 overflow-y-auto">
                          {getSelectedTeamMemberNames().map((name, idx) => (
                            ...
                            >
                              ...
                              <button
                                type="button"
                                onClick={() => removeAssignee(formData.assignedTeamMembers[idx])}
                    ...
                    >
                      ...
                      <svg className="w-4 h-4 transition-transform text-gray-400 shrink-0 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {showAssigneeDropdown && (
                      <div className="absolute z-20 w-[250px] mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                        {teamMembers.length === 0 ? (
                          ...
                          <div className="p-1">
                            {teamMembers.map((tm) => (
                              <label key={tm._id} className="flex items-center p-2 hover:bg-gray-50 cursor-pointer rounded-md transition-colors min-w-0">
                                <input
                                  type="checkbox"
                                ...
                                <div className="flex items-center min-w-0 flex-1">
                                  ...
                                  <span className="text-sm font-medium text-gray-700 truncate min-w-0">{tm.name}</span>
                                </div>
                              </label>
                            ))}
                          </div>
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-2 mt-4 pt-3 border-t border-gray-100">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium shadow-sm disabled:opacity-60"
                >
